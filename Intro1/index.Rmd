...
datos |> 
  ...
  labs(title = "Incidencia acumulada Covid-19. Comparativo Argentina - USA (2020-2023)",
...
datos |> 
  ...
  filter(iso3c == "ARG" & date >= "2020-03-01") |> 
  arrange(date) |> 
DT::datatable(
...
- **|>** es un conector incluido recientemente dentro del lenguaje R. Similar al conector **%>%** del ecosistema _tidyverse_. Son denominados "tuberías" (pipe en inglés). Se utilizan para conectar cada función de R y trasladan el termino de la izquierda hacia la función de la derecha.
En este caso define que la tarea que realice la función **_filter()_** (2da línea) se hará sobre los datos de la tabla **datos**.
...
-  Las funciones de capas geométricas poseen argumentos que se pueden configurar, como **linewidth** que determina el grosor de la línea de este gráfico.
...
## Para hacer en su equipo

### Realicen un gráfico similar comparando mortalidad cada millon de habitantes entre Argentina y Brasil (BRA)

#### Nota: la finalidad de este ejercicio es trabajar con R. Para realizar una comparación correcta antes deberíamos ajustar la tasa de mortalidad por edad en cada país.