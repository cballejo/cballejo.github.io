...
::: incremental
-   Comprender los conceptos fundamentales sobre estructura y organización de los datos en salud.
-   Instalar y configurar el entorno de trabajo con R, RStudio, Rtools y paquetes básicos.
-   Explorar características relevantes del lenguaje R y tidyverse
...
-   Calcular indicadores epidemiológicos simples ( incidencia, etc).
:::
...
Llamamos “datos ordenados” (tidy data) cuando:
...
::: {style="font-size: 80%;"}
-   Cada observación está en una fila y pertenece a una unidad de observación.
:::
...
:::: {.column width="55%"}
::: {style="font-size: 80%;"}
**GUI (Graphical User Interface):** curva de aprendizaje inicial muy rápida. Luego pierde capacidad de realizar tareas nuevas.
...
::: {.fragment .fade-in-then-semi-out}
El lenguaje R es es un lenguaje de programación interpretado, orientado a objetos, multiplataforma y open source pensado para el manejo de datos estadísticos.
:::

::: {.fragment .fade-in-then-semi-out}
Se considera la *lingua franca* de la estadística.
:::

::: {.fragment .fade-in-then-semi-out}
El objetivo es comenzar a *“decir cosas con los datos”* como un usuario del lenguaje (inicialmente no como un programador).
:::
...
-   **RStudio** es un *entorno de desarrollo integrado* (**IDE**) para el lenguaje estadístico R.

. . .

-   Incluye una consola, un editor de sintaxis que apoya la ejecución de código, así como herramientas para el manejo de paquetes (librerías), gráficos y la gestión del espacio de trabajo.
...
::: {.fragment .fade-in-then-semi-out}
-   Reutiliza las estructuras de datos mediante el uso de tuberías
:::
...
::: {.fragment .fade-in-then-semi-out}
-   Diseñada para humanos incorporando una gramática específica al lenguaje (que ya posee una sintaxis estricta y una semantica que le otorga significado)
:::
...
Un dataframe (marco de datos), es similar a una tabla de datos construida por una colección de vectores ubicados verticalmente que mantienen la integridad de sus filas
...
-   También se pueden hacer búsquedas en Google, foros especializados, como por ejemplo Stack Overflow o Rpubs, canales de slack y más recientemente en IAs como chatGPT o Google Gemini.
...
3.  Filtramos observaciones con `edad` mayor a 20 años
...
- Calcular estadísticos resumen de edad, sexo, etc.
- Calcular indicadores (incidencia acumulada y letalidad)
- Elaborar al menos dos gráficos descriptivos.
- Presentar el script comentado y un breve resumen de resultados (1–2 páginas).
...
- [Tabla de datos SISA dengue]()

Comprimir la carpeta con el proyecto de RStudio y el script con la resolución del trabajo práctico.
